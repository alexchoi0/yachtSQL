...
fn setup_users_table(executor: &mut QueryExecutor) {
    executor
        .execute_sql("CREATE TABLE users (id INT, first_name STRING, last_name STRING, email STRING, name STRING)")
        .unwrap();
    executor
...
fn setup_data_table(executor: &mut QueryExecutor) {
    executor
        .execute_sql("CREATE TABLE data (id INT, name STRING, text STRING)")
        .unwrap();
    executor
...
fn setup_contacts_table(executor: &mut QueryExecutor) {
    executor
        .execute_sql("CREATE TABLE contacts (id INT, phone STRING)")
        .unwrap();
    executor
...
fn test_initcap() {
    let mut executor = create_executor();
    executor
        .execute_sql("CREATE TABLE books (title STRING)")
        .unwrap();
    executor
        .execute_sql("INSERT INTO books VALUES ('the great gatsby')")
        .unwrap();

    let result = executor
        .execute_sql("SELECT INITCAP(title) AS cap_title FROM books")
        .unwrap();

    assert_eq!(result.num_rows(), 1);
    assert_eq!(get_string(&result, "cap_title", 0), "The Great Gatsby");
}
...
fn test_like_single_char() {
    ...
    let result = executor
        .execute_sql("SELECT * FROM users WHERE first_name LIKE 'J_ne'")
        .unwrap();
...
fn test_strpos() {
    ...
    let result = executor
        .execute_sql("SELECT STRPOS('hello world', 'world') AS pos")
        .unwrap();
