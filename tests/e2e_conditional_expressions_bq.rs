...
#[test]
fn test_ifnull_basic() {
    let mut executor = setup_executor();
    executor
        .execute_sql("CREATE TABLE data (value INT64)")
        .unwrap();
    executor
        .execute_sql("INSERT INTO data VALUES (NULL), (10)")
        .unwrap();

    let result = executor
        .execute_sql("SELECT IFNULL(value, 0) as result FROM data ORDER BY result")
        .unwrap();
...
#[test]
fn test_case_conditional_score() {
    let mut executor = setup_executor();
    executor
        .execute_sql("CREATE TABLE students (name STRING, score INT64)")
        .unwrap();
...
#[test]
fn test_case_with_coalesce_null_check() {
    let mut executor = setup_executor();
    executor
        .execute_sql("CREATE TABLE data (value INT64)")
        .unwrap();
    executor
        .execute_sql("INSERT INTO data VALUES (NULL), (5)")
        .unwrap();

    let result = executor
        .execute_sql("SELECT CASE WHEN COALESCE(value, 0) > 0 THEN 'positive' ELSE 'zero_or_null' END as result FROM data ORDER BY value NULLS FIRST")
        .unwrap();

    assert_eq!(result.num_rows(), 2);
    let col = result.column(0).unwrap();
    assert_eq!(col.get(0).unwrap().as_str().unwrap(), "zero_or_null");
    assert_eq!(col.get(1).unwrap().as_str().unwrap(), "positive");
}
