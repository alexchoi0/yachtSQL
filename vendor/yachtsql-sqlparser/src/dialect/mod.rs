...
pub trait Dialect: Debug + Any {
    ...
    /// Returns true if the dialect uses `WHILE condition DO ... END WHILE` syntax
    /// instead of `WHILE condition BEGIN ... END`.
    fn supports_while_do_end_while(&self) -> bool {
        false
    }

    /// Returns true if the dialect uses `LOOP ... END LOOP` syntax.
    fn supports_loop_end_loop(&self) -> bool {
        false
    }

    /// Returns true if the dialect uses `FOR var IN range DO ... END FOR` syntax.
    fn supports_for_do_end_for(&self) -> bool {
        false
    }

    /// Returns true if the dialect uses `REPEAT ... UNTIL condition END REPEAT` syntax.
