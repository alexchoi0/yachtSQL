...
pub const POSTGRES_VERSION: &str = "16.0";

pub fn default_capabilities() -> CapabilitySnapshot {
    snapshot().expect("postgres manifest should be internally consistent")
}
...
fn core_features() -> Vec<FeatureToggle> {
    vec![
        ...
        (F784_SAVEPOINT_STATEMENT, true),
        (F785_ROLLBACK_TO_SAVEPOINT_STATEMENT, true),
        (F786_SAVEPOINTS, true),
...
fn optional_features() -> Vec<FeatureToggle> {
    vec![
        ...
        (T176_SEQUENCE_GENERATOR, true),
        (T301_SAVEPOINTS, true),
        (T321_TRANSACTION_ISOLATION, true),
...
fn extension_features() -> Vec<FeatureToggle> {
    vec![
        ...
        (PG_SERIAL, true),
        (PG_ILIKE, true),
        (PG_DOUBLE_COLON_CAST, true),
        (PG_ARRAY_LITERALS, true),
        (PG_JSONB_TYPE, true),
        (PG_JSONB_OPERATORS, true),
        (PG_RETURNING_STANDARDIZATION, false),
    ]
}
