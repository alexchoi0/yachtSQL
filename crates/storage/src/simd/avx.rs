...
unsafe fn sum_i64_masked_avx2(data: &[i64], nulls: &[u8]) -> i64 {
    ...
    for (chunk_data, chunk_nulls) in chunks_data.zip(chunks_nulls) {
        ...
        let mask_0 = chunk_nulls[0] as i64;
        let mask_1 = chunk_nulls[1] as i64;
        let mask_2 = chunk_nulls[2] as i64;
        let mask_3 = chunk_nulls[3] as i64;
        let mask_vec = _mm256_set_epi64x(mask_3, mask_2, mask_1, mask_0);

        let masked = _mm256_mul_epi64(data_vec, mask_vec);
...
unsafe fn sum_f64_masked_avx2(data: &[f64], nulls: &[u8]) -> f64 {
    ...
    for (chunk_data, chunk_nulls) in chunks_data.zip(chunks_nulls) {
        ...
        let mask_2 = chunk_nulls[2] as f64;
        let mask_3 = chunk_nulls[3] as f64;
        let mask_vec = _mm256_set_pd(mask_3, mask_2, mask_1, mask_0);

        let masked = _mm256_mul_pd(data_vec, mask_vec);
...
unsafe fn min_i64_avx2(data: &[i64]) -> Option<i64> {
    ...
    for chunk in chunks {
        let v = _mm256_loadu_si256(chunk.as_ptr() as *const __m256i);
        min_vec = _mm256_min_epi64(min_vec, v);
    }
...
unsafe fn max_i64_avx2(data: &[i64]) -> Option<i64> {
    ...
    for chunk in chunks {
        let v = _mm256_loadu_si256(chunk.as_ptr() as *const __m256i);
        max_vec = _mm256_max_epi64(max_vec, v);
    }
...
pub fn avg_i64_masked(data: &[i64], nulls: &[u8]) -> Option<f64> {
    ...
    let count = count_nonzero_i64(nulls);
