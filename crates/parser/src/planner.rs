...
impl<'a, C: CatalogProvider> Planner<'a, C> {
    ...
    pub fn plan_statement(&self, stmt: &Statement) -> Result<LogicalPlan> {
        match stmt {
            ...
            Statement::CreateFunction(create_func) => self.plan_create_function(create_func),
    ...
    fn plan_create_function(&self, create: &ast::CreateFunction) -> Result<LogicalPlan> {
        ...
        let body = match language.as_str() {
            "JAVASCRIPT" | "JS" => {
                let js_code = self.extract_string_from_function_body(&create.function_body)?;
            ...
            "SQL" | "" => {
                ...
                let empty_schema = PlanSchema::new();
                let planned_expr = ExprPlanner::plan_expr(expr, &empty_schema)?;
                FunctionBody::Sql(Box::new(planned_expr))
            }
            _ => {
                return Err(Error::unsupported(format!(
            ...
            .map(|args| {
                args.iter()
                    .filter_map(|arg| {
                        ...
                        let data_type = self.sql_type_to_data_type(&arg.data_type);
                        Some(FunctionArg {
                            name: param_name,
                            data_type,
                            default: None,
    ...
    fn extract_string_from_function_body(
        &self,
        function_body: &Option<ast::CreateFunctionBody>,
    ) -> Result<String> {
        let expr = match function_body {
            Some(ast::CreateFunctionBody::AsBeforeOptions(expr)) => expr,
            Some(ast::CreateFunctionBody::AsAfterOptions(expr)) => expr,
            _ => {
                return Err(Error::invalid_query(
